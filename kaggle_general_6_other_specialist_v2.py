"""Train a focal-loss OTHER-vs-rest specialist beside the best 6-class AST.
...
V2 differences:
  - focal loss focuses the binary specialist on hard OTHER/non-OTHER examples,
  - the base model remains frozen except for the final AST blocks opened later,
  - the specialist/fusion rule is selected only with validation Macro-F1,
  - the independent test split is evaluated exactly once after selection.
  - five fixed windows are combined by majority vote; mean probability only
    breaks vote ties.
"""
...
OUTPUT_STEM = "general_6_other_specialist_focal_vote_v3"
...
def majority_vote_scores(window_probabilities: np.ndarray) -> np.ndarray:
    """Return record scores whose argmax is the window-majority decision.

    Vote share is the primary signal.  The mean class probability is scaled
    below one vote, so it can break a vote tie without overturning a strict
    majority.  Returning a normalized score vector keeps the downstream
    OTHER-specialist fusion unchanged.
    """
    probabilities = np.asarray(window_probabilities, dtype=np.float64)
    if probabilities.ndim != 2 or probabilities.shape[0] == 0:
        raise ValueError("window_probabilities must be a non-empty 2D array")
    if probabilities.shape[1] != len(LABELS):
        raise ValueError("window probability columns must match LABELS")
    row_sums = probabilities.sum(axis=1, keepdims=True)
    if not np.all(np.isfinite(probabilities)) or np.any(probabilities < 0) or np.any(row_sums <= 0):
        raise ValueError("window probabilities must be finite and non-negative")
    probabilities = probabilities / row_sums
    votes = np.bincount(probabilities.argmax(1), minlength=len(LABELS)).astype(np.float64)
    # A complete probability tie-break contributes less than one full vote.
    scores = votes + probabilities.mean(0) * 0.999
...
def focal_cross_entropy(
    ...
    """Binary focal CE; balanced sampler handles class frequency.

    The loss is computed in float32 so mixed precision cannot overflow.
    Easy examples receive a small multiplier while difficult examples keep
    most of their cross-entropy contribution.
    """
    logits = logits.float()
    cross_entropy = F.cross_entropy(logits, targets, reduction="none")
    true_probability = torch.softmax(logits, dim=1).gather(1, targets[:, None]).squeeze(1)
    focal_factor = (1.0 - true_probability).clamp_min(1e-6).pow(gamma)
...
def evaluate(frame: pd.DataFrame):
    ...
    for index, row in frame.iterrows():
        ...
        window_base_probabilities = torch.softmax(base_logits.float(), 1).cpu().numpy()
        base_probabilities.append(majority_vote_scores(window_base_probabilities))
...
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    for step, (first, second, target) in enumerate(loader, 1):
        ...
        with torch.amp.autocast("cuda"):
            ...
            loss = (
                focal_cross_entropy(first_logits, target)
                + focal_cross_entropy(second_logits, target)
            ) / (2 * ACCUMULATION)
    ...
    if selected_f1 > best_validation_f1 + 1e-5:
        ...
        best_validation_base_f1 = float(base_f1)
        best_epoch = epoch
        best_rule = {
...
result = {
    "experiment": "general_6_other_specialist_focal_vote_v3",
    "base_model": BASE_STEM,
    "selection_uses_test_set": False,
    "specialist_loss": "focal_cross_entropy",
    "focal_gamma": FOCAL_GAMMA,
    "window_aggregation": "5_window_majority_vote_mean_probability_tie_break",
    "safety_gate": "Specialist is disabled unless validation Macro-F1 improves",
    "best_epoch": best_epoch,
    "best_validation_base_macro_f1": best_validation_base_f1,
    "best_validation_macro_f1": best_validation_f1,
    "validation_macro_f1_gain": best_validation_f1 - best_validation_base_f1,
    "selected_rule": best_rule,
    "independent_test_base": base_metrics,
    "independent_test_selected": selected_metrics,
    "independent_test_gain": {
        "accuracy": selected_metrics["accuracy"] - base_metrics["accuracy"],
        "macro_f1": selected_metrics["macro_f1"] - base_metrics["macro_f1"],
    },
}
...
(OUTPUT / "config.json").write_text(
    json.dumps(
        {
            "architecture": "AST 6-class cosine head + focal binary OTHER specialist",
            "labels": LABELS,
            **result,
        },
        indent=2,
